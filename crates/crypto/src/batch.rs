...
mod tests {
    ...
    #[test]
    fn batch_verify_timing_comparison() {
        let count = 50;
        let signers: Vec<DilithiumSigner> =
        ...
        for i in 0..count {
            let _ = mv
                .verify(signers[i].public_key(), &messages[i], &sigs[i])
                .unwrap();
        }
        ...
        let items: Vec<VerifyItem> = signers
            ...
            .collect();
        let start = std::time::Instant::now();
        let _ = mv.verify_batch(&items).unwrap();
        let batch = start.elapsed();

        println!(
            "\n[M5-C2 batch timing] {count} sigs — sequential: {:.1}ms, batch: {:.1}ms, speedup: {:.2}x",
