...
impl FilterRegistry {
    ...
    pub fn new_filter(&self, kind: FilterKind, current_block: u64) -> String {
        ...
        self.filters.write().insert(id.clone(), entry);
        id
    }
    ...
    /// Returns the number of active filters (useful for tests/metrics).
    #[cfg(test)]
...
mod tests {
    ...
    fn new_filter_returns_hex_id() {
        ...
        let id = reg.new_filter(FilterKind::Block, 0);
    ...
    fn filter_ids_are_monotonic() {
        let reg = FilterRegistry::new();
        let id1 = reg.new_filter(FilterKind::Block, 0);
        let id2 = reg.new_filter(FilterKind::Block, 0);
    ...
    fn uninstall_existing_filter() {
        let reg = FilterRegistry::new();
        let id = reg.new_filter(FilterKind::Block, 10);
    ...
    fn get_filter_info_returns_correct_data() {
        let reg = FilterRegistry::new();
        let id = reg.new_filter(FilterKind::Block, 42);
    ...
    fn get_filter_info_for_log_filter() {
        ...
        let reg = FilterRegistry::new();
        let id = reg.new_filter(FilterKind::Log(raw), 5);
        let (is_log, last_poll) = reg.get_filter_info(&id).unwrap();
        assert!(is_log);
    ...
    fn get_log_filter_returns_raw_for_log_kind() {
        ...
        .unwrap();
        let reg = FilterRegistry::new();
        let id = reg.new_filter(FilterKind::Log(raw), 1);
    ...
    fn cleanup_keeps_fresh_filters() {
        ...
        let _id = reg.new_filter(FilterKind::Block, 0);
        reg.cleanup_expired();
        assert_eq!(reg.len(), 1);
    }
}
