...
impl DownloadedSnapshot {
    fn new(datadir: &Path) -> Result<Self, NodeError> {
        let file = tempfile::Builder::new()
            .prefix("checkpoint_snapshot-")
            .suffix(".jsonl")
            .tempfile_in(datadir)
            .map_err(|e| NodeError::Startup(format!("create snapshot download file: {e}")))?;
        Ok(Self { file })
    }
    ...
    fn reader(&self) -> Result<std::fs::File, NodeError> {
        let mut file = self
            ...
            .as_file()
            .try_clone()
            .map_err(|e| NodeError::Startup(format!("clone snapshot download file: {e}")))?;
        file.seek(SeekFrom::Start(0))
            .map_err(|e| NodeError::Startup(format!("rewind snapshot download file: {e}")))?;
        Ok(file)
    }
}
...
pub async fn checkpoint_sync<S: KvStore>(
    ...
    download_snapshot(url, snapshot.file.as_file()).await?;
...
async fn download_snapshot(url: &str, dest: &std::fs::File) -> Result<(), NodeError> {
    let output_file = dest
        .try_clone()
        .map_err(|e| NodeError::Startup(format!("clone snapshot download file: {e}")))?;
...
mod tests {
    ...
    fn downloaded_snapshot_guard_removes_file_on_drop() {
        ...
        let path = {
            let snapshot = DownloadedSnapshot::new(dir.path()).unwrap();
            let mut file = snapshot.file.as_file().try_clone().unwrap();
            file.write_all(b"partial snapshot").unwrap();
    ...
    #[test]
    fn downloaded_snapshot_does_not_reuse_predictable_paths() {
        let dir = tempfile::tempdir().unwrap();
        let legacy_path = dir
            .path()
            .join(format!("checkpoint_snapshot-{}.jsonl", std::process::id()));
        std::fs::write(&legacy_path, b"existing file").unwrap();

        let first_path = {
            let snapshot = DownloadedSnapshot::new(dir.path()).unwrap();
            assert_ne!(snapshot.file.path(), legacy_path);
            assert!(snapshot.file.path().exists());
            snapshot.file.path().to_path_buf()
        };
        let second_path = {
            let snapshot = DownloadedSnapshot::new(dir.path()).unwrap();
        ...
        assert_eq!(std::fs::read(legacy_path).unwrap(), b"existing file");
    }
}
